...
public class PlayerNormalAttackState : PlayerStateBase
{
    ...
    public override void Update()
    {
        ...
        if (!IsAnimationEnd() && playerController.inputSystem.Player.Fire.triggered && !enterNextAttack && !playerController.mouseOpen)
        {
        ...
        if (IsAnimationEnd())
        {
            if(enterNextAttack)
            {
                ...
                if (playerModel.characterStats.skillConfig.currentNormalAttackIndex > playerModel.characterStats.skillConfig.normalAttackDamageMultiple.Length)
                {
                    playerModel.characterStats.skillConfig.currentNormalAttackIndex = 1;
                }
                //切换到普通攻击状态
